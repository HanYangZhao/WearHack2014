...
void compare(int input, int definition, String alphabet[] ,  int alphabetCounter[], String letter, int index);
//void queueExtract(QueueList <String>  queue, int alphabetCounter[], String alphabet[]);
String alphabetExtract( int alphabetCounter[], String alphabet[]);
...
void loop(){
 ...
 for (int i = 0 ; i < 5 ; i++){
  ...
  for ( int j = 0 ; j < 5 ; j++){   
    for (int i = 0 ; i < 23 ; i++){
      compare(reading[j],matrix[i][j], alphabet, alphabetCounter, alphabet[i], i);
      
    }
  }
  
  //queueExtract(potentialLetters, alphabetCounter, alphabet);
  Serial.println(alphabetExtract(alphabetCounter, alphabet));
...
String alphabetExtract( int alphabetCounter[], String alphabet[]){
   String result = alphabet[0];
   int maxSoFar = alphabetCounter[0];
   for ( int i = 0 ; i < 23 ; i++ ){
       
       // Serial.print(alphabet[i]);
       // Serial.print(":");
       // Serial.println(alphabetCounter[i]);
        if (alphabetCounter[i] > maxSoFar){
          maxSoFar = alphabetCounter[i];
          result = alphabet[i];   
        }
